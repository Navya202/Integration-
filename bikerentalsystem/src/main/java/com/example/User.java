...
public class User {
    ...
    //         if (rental.getBikes().contains(bike) && rental.isOngoing()) {
    //             rental.endRental();
    ...
    public void returnBike(int bikeId) {
        for (Rental rental : rentalHistory) {
            for (Bike bike : rental.getBikes()) {
                if (bike.getBikeId() == bikeId && rental.isOngoing()) {
                    rental.endRental();
                    bike.setAvailable(true);
                    System.out.println("Bike returned: " + bike);
                    return;
        ...
        System.out.println("No ongoing rental found for bike with ID: " + bikeId);
    }
    ...
    public void viewRentalCharges(Bike bike) {
        for (Rental rental : rentalHistory) {
            if (rental.getBikes().contains(bike)) {
                System.out.println("Rental charges: " + rental.getRentalCharges());
                return;
            }
        }
